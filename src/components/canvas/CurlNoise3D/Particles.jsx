...
export function Particles({ speed = 100, fov = 20, aperture = 1.8, focus = 5.1, curl = 0.25, size = 420, scroll = 0, interactive = true, ...props }) {
  ...
  useFrame((state) => {
    ...
    // Check if scroll is a MotionValue (framer-motion) or a raw number
    const currentScroll = scroll?.get ? scroll.get() : scroll;
    simRef.current.uniforms.uScroll.value = THREE.MathUtils.lerp(simRef.current.uniforms.uScroll.value, currentScroll, 0.1)
