...
export class SimulationMaterial extends THREE.ShaderMaterial {
  constructor() {
    const format = typeof Float32Array !== 'undefined' ? THREE.FloatType : THREE.HalfFloatType
    const positionsTexture = new THREE.DataTexture(getSphere(512 * 512, 128), 512, 512, THREE.RGBAFormat, format)
    ...
    super({
      ...
      void main() {
        ...
        // Global scroll-based scattering
        vec3 scrollScatter = curl(finalPos * 1.2 + t) * uScroll * 4.5;
        finalPos += scrollScatter;

        // Interaksi Mouse (Cylinder Raycast)
        vec3 rayDir = normalize(uMouse - uCameraPos);
        vec3 pToCam = finalPos - uCameraPos;
        float dist = length(cross(pToCam, rayDir)); 
        
        float radius = 0.6; 
        float force = smoothstep(radius, 0.0, dist);

        if (force > 0.0) {
          vec3 closestPoint = uCameraPos + rayDir * dot(pToCam, rayDir);
          vec3 away = normalize(finalPos - closestPoint + vec3(0.001));
          vec3 scatter = curl(finalPos * 4.0 + t); 
          float smoothHover = smoothstep(0.0, 1.0, uHover);
          vec3 displacement = (away * 0.1 + scatter * 1.5) * force * smoothHover;
